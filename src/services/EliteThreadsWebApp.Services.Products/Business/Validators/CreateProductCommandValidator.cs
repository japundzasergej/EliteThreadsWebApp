...
namespace EliteThreadsWebApp.Services.Products.Business.Validators
{
    public class CreateProductCommandValidator : AbstractValidator<CreateProductCommand>
    {
        public CreateProductCommandValidator()
        {
            ...
            RuleFor(command => command.ProductDTO.ImageList)
                .NotEmpty()
                .Must(p => p.Count >= 3 && p.Count <= 5)
                .WithMessage("Provide between 3 and 5 images.");
            ...
            ;
            RuleFor(command => command.ProductDTO.SubcategoryId).NotEmpty();
        }
    }
}
